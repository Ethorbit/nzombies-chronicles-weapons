...
SWEP.Primary.DamageRange = NZCoDRange(15.0)
...
SWEP.Primary.Damage		= 380 -- Base damage per bullet
...
function SWEP:OnPaP()
	...
	self.Primary.Damage = 580
	self.Ispackapunched = 1
	self.Primary.ClipSize = 28
	self.Primary.MaxAmmo = 84
	--self.Shotgun = false
	self.SequenceRateOverride = {[ACT_VM_RELOAD] = 1.8, [ACT_VM_RELOAD_EMPTY] = 1.8} --Like above but changes animation length to a target
	self.SequenceRateOverrideScaled = {[ACT_VM_RELOAD] = 1.8, [ACT_VM_RELOAD_EMPTY] = 1.8} --Like above but scales animation length rather than being absolute
	self:ClearStatCache()
