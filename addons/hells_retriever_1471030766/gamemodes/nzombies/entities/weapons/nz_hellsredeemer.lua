...
if (CLIENT) then
	SWEP.Slot = 0
	SWEP.SlotPos = 5
	SWEP.PrintName = "Hell's Redeemer"
...
function SWEP:Initialize()
	timer.Simple(0.1, function()
		...
		self.Owner:SelectWeapon("nz_hellsredeemer")
	end)
...
function SWEP:ThrowGrenade(force)
	if SERVER then
		...
		nade.Upgraded = true
		ParticleEffectAttach("redeemer_trail1", PATTACH_ABSORIGIN_FOLLOW, nade, 0)
...
function SWEP:Deploy()
	if self.FirstRaise == true then
		...
		timer.Create("chargetimer", 1.5, 2, function()
			self.Charged = self.Charged or 0
			self.Charged = self.Charged + 1
			effectdata:SetOrigin(self.Owner:GetPos() + Vector(0,0,4))
			util.Effect("redeemer_chargering", effectdata)
			ParticleEffectAttach("redeemer_charge", PATTACH_ABSORIGIN_FOLLOW, self.Owner, 0)
		end)
		effectdata:SetOrigin(self.Owner:GetPos() + Vector(0,0,4))
		util.Effect("redeemer_chargering", effectdata)
		ParticleEffectAttach("redeemer_charge", PATTACH_ABSORIGIN_FOLLOW, self.Owner, 0)
