...
function SWEP.Reload() -- Definitely don't want flamethrowers to reload
end
...
function SWEP:Think2( ... )
	...
	if (SERVER) then
		...
		-- Overheat functionality
		-------------------------------------
		if (clip <= 0) then
			self.overheated = true
			self:SetNextPrimaryFire(CurTime() + 99999) 
		end

		if (clip >= self.Primary.DefaultClip) then 
		...
		---------------------------------------
		-- Regenerate ammo
		if (self.lastFire == nil or CurTime() > self.lastFire) then
			if clip < self.Primary.DefaultClip then
				if (self.nextregen == nil or CurTime() > self.nextregen) then
