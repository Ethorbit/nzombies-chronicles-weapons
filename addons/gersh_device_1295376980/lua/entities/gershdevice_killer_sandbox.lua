...
function ENT:Think()
    ...
    for _, target in pairs(ents.FindInSphere(self:GetPos(), 1024)) do
        ...
        if target:IsNPC() || target.Type == "nextbot" then
	...
	for _, target in pairs(ents.FindInSphere(self:GetPos(), 54)) do
		if SERVER then
			if IsValid(target) and (target:IsNPC()) or target.Type == "nextbot" then
