...
hook.Add("EntityEmitSound", "BabySounds", function(data)
	if data and IsValid(data.Entity) and data.Entity:GetNWString("IsBaby") == "true" and !table.HasValue( babyDieTable, data.OriginalSoundName ) and !table.HasValue( kickSoundTable, data.OriginalSoundName ) and data.OriginalSoundName != "weapons/shrinkray/effects/evt_unshrink.wav" and data.OriginalSoundName != "weapons/shrinkray/effects/evt_shrink.wav" then
		data.Pitch = 168
		return true
	end
end)
