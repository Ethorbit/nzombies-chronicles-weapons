...
SWEP.ViewModel				= "models/weapons/wavegun/nzc_c_zapwave_gun.mdl"	-- Weapon view model
SWEP.WorldModel				= "models/weapons/wavegun/nzc_w_zapguns.mdl"	-- Weapon world model
SWEP.WorldModelCombined = "models/weapons/wavegun/nzc_w_wavegun.mdl"
...
function SWEP:FireRocket()
	...
	if SERVER then
		if self:GetNWString("dw_state") == "combined" then
			...
			local pos = self.Owner:GetShootPos() + self.Owner:GetUp()* -8 + self.Owner:GetRight()*8
			if self:GetIronSights() == false then
				pos = self.Owner:GetShootPos() + self.Owner:GetUp()* -8 + self.Owner:GetRight()*8
			else
				pos = self.Owner:GetShootPos() + self.Owner:GetUp()* -8
			end
			...
			if self.Ispackapunched > 0 then
				ParticleEffectAttach("wgun_muzzle_pap", PATTACH_POINT_FOLLOW, self.Owner:GetViewModel(), 2)
			else
				ParticleEffectAttach("wgun_muzzle", PATTACH_POINT_FOLLOW, self.Owner:GetViewModel(), 2)
			end
...
function SWEP:ToggleDWCombine()
	...
	if self.CanChangeStates == true then
		...
		if self:GetNWString("dw_state") == "dw" then --combine weps
			...
			self.WorldModel = "models/weapons/wavegun/nzc_w_wavegun.mdl"
		...
		elseif self:GetNWString("dw_state") == "combined" then --detach weps
			self.WorldModel = "models/weapons/wavegun/nzc_w_zapguns.mdl"
...
function SWEP:PreDrawViewModel( vm )
	if self.Ispackapunched == 1 then
		self.Owner:GetViewModel():SetSubMaterial(0, "models/weapons/common/bo1_pap_camo_large_c.vmt")
		self.Owner:GetViewModel():SetSubMaterial(2, "models/weapons/common/bo1_pap_camo_large_c.vmt")
	end
end
