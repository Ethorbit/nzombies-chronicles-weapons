...
SWEP.ViewModel			= "models/weapons/wunderwaffe/nzc_v_wunderwaffe.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/wunderwaffe/nzc_w_wunderwaffe.mdl" -- Weapon world model path
...
function SWEP:FireAnimationEvent(pos, ang, event, options)
	...
	if event == 9001 then -- Pullout
		...
		if self.Ispackapunched > 0 then
			for i = 1, math.Round(self:Clip1()/2) do
				if (bulb and isnumber(bulb[i])) then
	...
	if event == 9061 then -- Fire
		...
		if self.Ispackapunched == 1 then
			ParticleEffectAttach( "tesla_mflash_pap", PATTACH_POINT_FOLLOW, vm, 1)
		else
