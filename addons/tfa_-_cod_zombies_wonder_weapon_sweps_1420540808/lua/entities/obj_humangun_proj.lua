...
function ENT:OnCollide(ent,hitpos)
	...
	self:NextThink(CurTime())
		...
		if ent:IsNPC() and ent:GetHullType() != HULL_HUMAN then ent:TakeDamage(143,self.Owner,self.Weapon) end
        if c == "nz_zombie_boss_panzer" then
		        ent:TakeDamage(143,self.Owner,self.Weapon)
		elseif c == "nextbot_vrill_human" then
		util.BlastDamage(self,self.Owner,ent:GetPos(),150,200)
