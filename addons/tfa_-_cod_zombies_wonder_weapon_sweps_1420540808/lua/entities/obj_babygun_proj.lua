...
if SERVER then
	function ENT:OnCollide(ent,hitpos)
		...
		-- If we turn them into babies during instakill that could cause some problems
		-- so just do what every other weapon does and you know.. instakill them.
		if (nzPowerUps:IsPowerupActive("insta")) then
			for k,v in pairs (ents.FindInSphere(hitpos,200)) do
				if IsValid(v) and (v:IsNPC() or scripted_ents.GetType(v:GetClass()) == "nextbot") and v:Alive() then
					v:Kill()
				end
			end
		return end
                    ...
                    if v:GetNWString("IsBaby") == "false" then
						...
						timer.Simple(math.random(self.Timer,self.Timer + 0.15),function()
                            if v:IsValid() and v:OnGround() and v:GetNWString("IsBaby") == "true" then
								...
								v:RespawnZombie() -- Just so their collisions don't get fucked up
	...
	function ENT:StartTouch(ent)
		if ent:GetClass() == "wall_block_zombie" || ent:GetClass() == "wall_block" || ent:GetClass() == "invis_wall" || ent:GetClass() == "invis_wall_zombie" || ent:GetClass() == "invis_wall_block" then return end
		if self.Owner == ent then return end
