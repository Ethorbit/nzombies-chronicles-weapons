...
if SERVER then
	function ENT:OnCollide(ent,hitpos)
		...
		for k, v in pairs(ents.FindInSphere(hitpos, 1000)) do
		if scripted_ents.GetType( v:GetClass() ) == "nextbot" then
			if v != self.Owner and IsValid(v) then
				if v:GetNWString("IsInflating") ~= "true" and v:GetNWString("IsInflating") ~= "false" then v:SetNWString("IsInflating","false") end
				if v:GetNWString("IsInflating") == "false" then
					v:SetNWString("IsInflating","true")
