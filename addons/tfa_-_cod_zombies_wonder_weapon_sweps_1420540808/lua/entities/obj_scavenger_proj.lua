...
ENT.HasTimeout = true
ENT.HasWindup = true
ENT.WindupSND = "weapons/ubersniper/ubersniper_proj_windup.wav"
ENT.Winduptime = 0.5
ENT.TimeoutPeriod = 3
...
function ENT:Explosion()
			if CLIENT then return end			
					...
					if IsValid(self.Owner) and self.Owner:HasWeapon("tfa_scavenger") and self.Owner:GetWeapon("tfa_scavenger").Ispackapunched == 1 then
						self:EmitSound("weapons/ubersniper/ubersniper_explode_pap.wav", 350, 100)
						ParticleEffect("ubersniper_pap_base",self:GetPos(),Angle(0,0,0),nil)
					else
						self:EmitSound("weapons/ubersniper/ubersniper_explode.wav", 350, 100)
