...
self.Emitter 		= ParticleEmitter( self.Pos )	// Emitter must be there so you don't get an error			//
	...
	if (self.Scale<3) then
		sound.Play( "weapons/boom.wav", self.Pos, 100, 100 )
	end

	self.Mat=math.ceil(self.Radius)
	
	if self.Mat != nil && mats[self.Mat] != nil then
		if 	   mats[self.Mat][2]==nil then  self:Dust()
		elseif mats[self.Mat][2]==1 then	self:Dust()	
