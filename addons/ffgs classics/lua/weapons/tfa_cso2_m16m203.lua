...
SWEP.PrintName				= "M16M203"		-- Weapon name (Shown on HUD)
...
--[[WEAPON HANDLING]]--
SWEP.Primary.Sound = Sound("tfa_cso2_m16a4m203.1") -- This is the sound of the weapon, when you shoot.
...
SWEP.NZPaPName				= "M32 Grenade Lobber 406"
...
function SWEP:NZMaxAmmo()
	...
	if SERVER then
		...
		if (self:HasNZModifier("pap")) then
			self.Owner:SetAmmo(15, "CSOM16_NADE")
		else
			self.Owner:SetAmmo(5, "CSOM16_NADE")
		end
    end
end
...
--Range Related
SWEP.Primary.Range = 650 * 39.3701 * 8 / 3 -- The distance the bullet can travel in source units.  Set to -1 to autodetect based on damage/rpm.
...
--[[VIEWMODEL]]--
SWEP.ViewModel			= "models/weapons/tfa_cso2/c_m16m203.mdl" --Viewmodel path
...
--[[WORLDMODEL]]--
SWEP.WorldModel			= "models/weapons/tfa_cso2/w_m16m203.mdl" -- Weapon world model path
...
SWEP.IronSightsPos = Vector(5.9, -5.487, 2.0)
SWEP.IronSightsAng = Vector(0.209, 1.779, 0)
...
SWEP.BlowbackVector = Vector(-0.03,-1,0.0) --Vector to move bone <or root> relative to bone <or view> orientation.
...
SWEP.Secondary.Ammo = "CSOM16_NADE"
SWEP.Secondary.Sound = "tfa_cso2_m203.1"
...
SWEP.Secondary.Projectile = "tfa_exp_contact"
SWEP.Secondary.ProjectileModel = "models/weapons/tfa_cso2/w_40mm_grenade.mdl"
SWEP.Secondary.ProjectileVelocity = 250 * 16
SWEP.Secondary.Damage = 850
...
function SWEP:PrimaryAttack( ... )
	if self:GetSilenced() and not self.Owner:KeyDown(IN_USE) then
		if CurTime() > self:GetNextPrimaryFire() and self:GetStatus() == TFA.GetStatus("idle") and self:Clip2() > 0 then
...
function SWEP:Reload( ... )
	if self:GetSilenced() and not self.Owner:KeyDown(IN_USE) then
		if self:Ammo2() > 0 and self:GetStatus() == TFA.GetStatus("idle") and self:Clip2()  < self.Secondary.ClipSize then
