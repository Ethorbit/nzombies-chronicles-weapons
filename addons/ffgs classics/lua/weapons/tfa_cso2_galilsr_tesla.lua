...
SWEP.PrintName				= "Galil SR Tesla"		-- Weapon name (Shown on HUD)
...
--[[WEAPON HANDLING]]--
SWEP.Primary.Sound = Sound("tfa_cso2_galilsniper_tesla.1") -- This is the sound of the weapon, when you shoot.
SWEP.Primary.SilencedSound = nil -- This is the sound of the weapon, when silenced.
SWEP.Primary.Damage = 385 -- Damage, in standard damage points.
...
SWEP.NZPaPName				= "SuperRuler Wunderwaffe X4-80"
--SWEP.NZPaPReplacement 	= "tfa_cso2_galilsr_tesla"	-- If Pack-a-Punched, replace this gun with the entity class shown here.
...
function SWEP:PrimaryAttack(...)
	if (SERVER) then
		...
		if (self:CanPrimaryAttack() and IsValid(self.Owner) and self:Clip1() > 0 and CurTime() > self.ShockCooldown and CurTime() > self:GetNextPrimaryFire()) then
			local shockChance = math.random(1, 2)
			if (shockChance == 1) then
...
function SWEP:Reload(...)
	self.ShockCooldown = CurTime() + 3.5
...
--[[VIEWMODEL]]--
SWEP.ViewModel			= "models/weapons/tfa_cso2/c_galilsniper_tesla.mdl" --Viewmodel path
...
--[[WORLDMODEL]]--
SWEP.WorldModel			= "models/weapons/tfa_cso2/w_galilsniper_tesla.mdl" -- Weapon world model path
...
SWEP.RunSightsAng = Vector(-11.869, -20.129, 16.056)
...
SWEP.IronSightsPos = Vector(6.69, -3.312, 2.17)
SWEP.IronSightsAng = Vector(2.549, 1.256, 1.59)
--[[INSPECTION]]--
...
SWEP.BlowbackVector = Vector(-0.0,-2,-0.05)
...
SWEP.Blowback_Shell_Effect = ""--Which shell effect to use
...
SWEP.ViewModelBoneMods = {
	["ValveBiped.Bip01_R_Finger01"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(1.927, -8.336, 0) },
	["ValveBiped.Bip01_R_Finger0"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(-8.907, 2.493, 0) }
}
DEFINE_BASECLASS( SWEP.Base )
...
function SWEP:Deploy( ... )
	...
	timer.Simple(0, function()
		if IsValid(self) and self:VMIV() then
			ParticleEffectAttach( "cso2_galilsniper_tesla", PATTACH_POINT_FOLLOW, self.OwnerViewModel, 2 )
		end
	end)
	return true
end
