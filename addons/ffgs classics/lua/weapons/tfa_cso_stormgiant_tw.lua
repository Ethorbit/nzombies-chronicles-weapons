...
SWEP.Primary.Attacks = {
	{
		...
		['dmg'] = 4900, --This isn't overpowered enough, I swear!!
		...
		['end'] = 1.65, --time before next attack
		['hull'] = 256, --Hullsize
	...
	[ACT_VM_SECONDARYATTACK] = {
		{
			...
			["value"] = function(wep, viewmodel)
				if (wep.NextSecondaryAttack and CurTime() < wep.NextSecondaryAttack) then 
					...
					effects.BeamRingPoint(wep.Owner:GetPos(), 0.5, 200, 250, 30, 113, Color(255, 100, 100))
				...
				effects.BeamRingPoint(wep.Owner:GetPos(), 0.5, 300, 310, 30, 113, Color(255, 100, 100))
				...
				timer.Simple(0.3, function()
					...
					effects.BeamRingPoint(wep.Owner:GetPos(), 1, 300, 1500, 80, 1, Color(255, 100, 100))
					...
					timer.Simple(0.5, function()
						...
						for _,zmb in pairs(ents.FindInSphere(wep.Owner:GetPos(), 1400)) do
							if (IsValid(zmb) and zmb:IsValidZombie() and zmb:Health() > 0 and !zmb.NZBoss) then
								local theirSpeed = zmb:GetVelocity():Length2D()
