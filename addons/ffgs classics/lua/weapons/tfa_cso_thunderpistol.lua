...
function SWEP:OnPaP()
	...
	self.BlastRange = 200
	self.TracerName = "tra_thunderpistol_pap"
end
...
function SWEP:PrimaryAttack(...)
	if (self:CanPrimaryAttack() and IsValid(self:GetOwner())) then
		...
		local zombsKilled = 0
		if (endPos) then
			if (self:HasNZModifier("pap") and zombsKilled < 30 or !self:HasNZModifier("pap") and zombsKilled < 20) then
				entsInRange = ents.FindInBox(tr.HitPos + Vector(self.BlastRange, self.BlastRange, self.BlastRange), tr.HitPos - Vector(self.BlastRange, self.BlastRange, self.BlastRange))
				for _,v in pairs(entsInRange) do
					if (IsValid(v) and v:IsValidZombie()) then
						...
						if SERVER then
							...
							dmg:SetDamage(v:Health() * 2)
							dmg:SetAttacker(self:GetOwner())
							v:TakeDamageInfo(dmg)
						end

						if (zombsKilled < 3) then
