if SERVER then
	AddCSLuaFile("nz_trinity_knockback.lua")
	SWEP.Weight			= 1
...
if CLIENT then
	...
	SWEP.PrintName     	    = "Trinity Knockback"			
	SWEP.Slot				= 1
	SWEP.SlotPos			= 1
	SWEP.DrawAmmo			= false
...
SWEP.Author			= "Cooking code by Hidden"
SWEP.Contact		= ""
...
function SWEP:Deploy()
	...
	-- 		util.Effect( "exp_trinity_knockback", effectdata )
	-- 		self.FuckedUp = true
	-- 		--self:PrimaryAttack()
	-- 	end)
	-- 	self.CrossShrink = 50
...
function SWEP:ThrowGrenade(force)
	...
	if SERVER then
		local nade = ents.Create("trinity_knockback")
		...
		if self.Owner:HasPerk("widowswine") then
			...
			nade.ExplodeTime = CurTime() + 2.0
		else
			nade.ExplodeTime = CurTime() + ((self.ct+4.0) - ct2)
		end
