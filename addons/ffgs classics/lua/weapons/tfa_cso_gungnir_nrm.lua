...
SWEP.Secondary.Recoil = Angle(-10,0,0) --Recoil Dir

function SWEP:ElectricuteZombie(zombie)
...
function SWEP:PrimaryAttack(...)
	if (SERVER) then
		...
		if (IsValid(self.Owner) and self:Clip1() > 0 and CurTime() > self.ShockCooldown and CurTime() > self:GetNextPrimaryFire()) then
			self.ShockCooldown = CurTime() + 0.3

			local traceHit = util.TraceHull({
				start = self.Owner:GetShootPos(),
				endpos = self.Owner:GetShootPos() + ( self.Owner:GetAimVector() * 9999999 ),
			...
			if (zombie.killedbyffgsstaff) then return end

			timer.Simple(0.3, function()
				if (IsValid(zombie) and zombie:IsValidZombie()) then
					self:ElectricuteZombie(zombie)
				end
			end)
		end
	end
...
function SWEP:AltAttack()
	if CurTime() >  self:GetNextPrimaryFire() and TFA.Enum.ReadyStatus[ self:GetStatus() ] and not self:GetSprinting() and self:Ammo2() > 0 then
		...
		if SERVER and IsValid(self.Owner) then
			local zombies = ents.FindInSphere(self.Owner:GetPos(), 300)
			for _,v in pairs(zombies) do
				if (IsValid(v) and v:IsValidZombie()) then
					if v.loco then
						v.loco:JumpAcrossGap(v:GetPos() - v:GetAimVector() * 220, Vector(100, 100, 100))
					end
				end
			end
		end
...
function SWEP:NZMaxAmmo()
	...
	if (self:HasNZModifier("pap")) then
		self.Owner:SetAmmo( 10, "GUNGNIR_BLAST" ) 
	else
		self.Owner:SetAmmo( 5, "GUNGNIR_BLAST" ) 
	end
end

function SWEP:OnPaP()
	self.MuzzleFlashEffect = "pap_muzzleflash"
	self.Primary.MaxAmmo = 170
	self.Primary.RPM = 800
	self.Primary.RPM_Semi = 800
	self:ClearStatCache()
end
...
SWEP.VMAng = Vector(0,8,0) --The viewmodel angular offset, constantly.   Subtract this from any other modifications to viewmodel angle.
...
SWEP.Secondary.Ammo			= "GUNGNIR_BLAST" -- Self explanitory, ammo type.
