...
function SWEP:PrimaryAttack(...)
	if (self:CanPrimaryAttack() and IsValid(self:GetOwner())) then
		...
		if (IsValid(tr.Entity) and tr.Entity:IsValidZombie()) then
			tr.Entity:SetColor(Color(math.random(0, 255), math.random(0, 255), math.random(0, 255)))

			if (!timer.Exists("Uncoloring" .. tr.Entity:EntIndex())) then
				timer.Create("Uncoloring" .. tr.Entity:EntIndex(), 5, 1, function()
					if (IsValid(tr.Entity) and tr.Entity:Health() > 0) then
						tr.Entity:SetColor(Color(255, 255, 255))
					end
				end)
			end
		end
