...
--[[WEAPON HANDLING]]--
SWEP.Primary.Sound = Sound("tfa_cso2_deagle.1") -- This is the sound of the weapon, when you shoot.
...
function SWEP:OnPaP()
	...
	self.Primary.RPM = 480
	self.Primary.Damage = 260
	self.Primary.MaxAmmo = 55
end

-- nZombies Stuff
SWEP.NZWonderWeapon		= false	-- Is this a Wonder-Weapon? If true, only one player can have it at a time. Cheats aren't stopped, though.
SWEP.NZPaPName				= "Eagle Hawk"
...
--[[VIEWMODEL]]--
SWEP.ViewModel			= "models/weapons/tfa_cso2/c_deserteagle.mdl" --Viewmodel path
...
--[[WORLDMODEL]]--
SWEP.WorldModel			= "models/weapons/tfa_cso2/w_deserteagle.mdl" -- Weapon world model path
...
SWEP.IronSightsPos = Vector(6.159, -1.269, 2.92)
SWEP.IronSightsAng = Vector(-0.621, -0, 0)
...
SWEP.BlowbackBoneMods = {
	["CSO2_Deagle_YAGSIL"] = { scale = Vector(1, 1, 1), pos = Vector(0, 2.5, 0), angle = Angle(0, 0, 0) }
...
function SWEP:deserteagleloy( ... )
	BaseClass.deserteagleloy( self, ... )
	timer.Simple(0.01, function()
		if not IsValid(self) then return end
		if not self:VMIV() then return end
		self:CleanParticles()
		ParticleEffectAttach( "ss_deserteagle_fire_f", PATTACH_POINT_FOLLOW, self.OwnerViewModel, 5 )
		ParticleEffectAttach( "ss_deserteagle_fire_f", PATTACH_POINT_FOLLOW, self.OwnerViewModel, 6 )
	end)
	return true
end
