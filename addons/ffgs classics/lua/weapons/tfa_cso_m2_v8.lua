...
SWEP.PrintName				= "Artillery Cannon"		-- Weapon name (Shown on HUD)
...
SWEP.NZPaPName			= "Artillery Cannon"
...
-- Added by Ethorbit to explode target:

--SWEP.Primary.DamageType = 134217728
-- SWEP.Primary.ExplodeSurface = true
-- SWEP.Primary.ExplosionRadius = 400
...
function SWEP:OnPaP()
	...
	timer.Simple(0, function()
		if (self.Owner:GetAmmoCount(self:GetPrimaryAmmoType() or self.Primary.Ammo)) > self.Primary.MaxAmmo then
			self:NZMaxAmmo()
		end
	end)	
end
...
SWEP.ProjectileVelocity = nil --Entity to shoot's velocity
