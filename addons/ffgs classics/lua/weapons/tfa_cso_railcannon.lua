...
SWEP.Primary.Damage		= 25 --effective damage
...
function SWEP:OnPaP()
	...
	self.TracerName = "cso_tra_railcan2"
...
function SWEP:Think2(...)
	...
	if self:HasNZModifier("pap") then
		...
		elseif self:GetCharge() ~= 0 then
			if (self:GetCharge() == 2) then 
				self:SetClip1(math.Clamp(self:Clip1() - 3, 0, 24))
			end

			if (self:GetCharge() == 3) then
				self:SetClip1(math.Clamp(self:Clip1() - 6, 0, 24))
			end
