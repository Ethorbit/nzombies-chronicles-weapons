...
SWEP.PrintName				= "M4A1 Flash"		-- Weapon name (Shown on HUD)
...
--[[WEAPON HANDLING]]--
SWEP.Primary.Sound = Sound("tfa_cso2_m4a1.1") -- This is the sound of the weapon, when you shoot.
SWEP.Primary.SilencedSound = Sound("tfa_cso2_m4a1.2") -- This is the sound of the weapon, when silenced.
SWEP.Primary.Damage = 250 -- Damage, in sflashdard damage points.
...
--SWEP.NZRePaPText		= "upgrade weapon"	-- When RePaPing, what should be shown? Example: Press E to your text here for 2000 points.
SWEP.NZPaPName				= "M8A2 Wunderwaffe"
--SWEP.NZPaPReplacement 	= "tfa_cso2_m4a1_flash"	-- If Pack-a-Punched, replace this gun with the entity class shown here.
...
--Range Related
SWEP.Primary.Range = 500 * 39.3701 * 8 / 3 -- The disflashce the bullet can travel in source units.  Set to -1 to autodetect based on damage/rpm.
...
--[[VIEWMODEL]]--
SWEP.ViewModel			= "models/weapons/tfa_cso2/c_m4a1flash.mdl" --Viewmodel path
...
SWEP.VMPos = Vector(0,-2,0) --The viewmodel positional offset, consflashtly.  Subtract this from any other modifications to viewmodel position.
SWEP.VMAng = Vector(0,0,0) --The viewmodel angular offset, consflashtly.   Subtract this from any other modifications to viewmodel angle.
SWEP.VMPos_Additive = false --Set to false for an easier time using VMPos. If true, VMPos will act as a consflasht delta ON TOP OF ironsights, run, whateverelse

--[[WORLDMODEL]]--
SWEP.WorldModel			= "models/weapons/tfa_cso2/w_m4a1flash.mdl" -- Weapon world model path
...
SWEP.IronSightsPos = Vector(5.52, -1.509, 1.63)
SWEP.IronSightsAng = Vector(0, -0.05, 2.5)

--[[INSPECTION]]--
SWEP.InspectPos = nil--Vector(0,0,0) --Replace with a vector, in style of ironsights position, to be used for inspection
SWEP.InspecfLashg = nil--Vector(0,0,0) --Replace with a vector, in style of ironsights angle, to be used for inspection
...
SWEP.BlowbackVector = Vector(-0.00,-2,0.0) --Vector to move bone <or root> relative to bone <or view> orientation.
...
function SWEP:PrimaryAttack(...)
	if (SERVER) then
		if (!isnumber(self.ShockCooldown)) then self.ShockCooldown = 0 end
		if (self:CanPrimaryAttack() and IsValid(self.Owner) and self:Clip1() > 0 and CurTime() > self.ShockCooldown and CurTime() > self:GetNextPrimaryFire()) then
			if (math.random(100) <= 20) then
				self.ShockCooldown = CurTime() + 0.3
				local zombie = self.Owner:GetEyeTrace().Entity
...
SWEP.ViewModelBoneMods = {
	["ValveBiped.Bip01_R_Hand"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(-8, 5, 0) }
}
...
function SWEP:Deploy( ... )
	...
	timer.Simple(0, function()
		if IsValid(self) and self:VMIV() then
			self:CleanParticles()
			ParticleEffectAttach( "ss_m4a1_fire_f", PATTACH_POINT_FOLLOW, self.OwnerViewModel, 3 )
			ParticleEffectAttach( "ss_m4a1_fire_f", PATTACH_POINT_FOLLOW, self.OwnerViewModel, 4 )
		end
	end)
	return true
end
