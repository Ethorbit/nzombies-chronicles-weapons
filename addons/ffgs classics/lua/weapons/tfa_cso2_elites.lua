...
--[[WEAPON HANDLING]]--
SWEP.Primary.Sound = Sound("tfa_cso2_elite.1") -- This is the sound of the weapon, when you shoot.
...
SWEP.NZPaPName				= "Dual Elitist 4-50"
...
function SWEP:OnPaP()
	...
	self.Primary.MaxAmmo = 330
	self.Primary.Damage = self.Primary.Damage * 2.5
	self.Primary.KickUp = 0.3 -- This is the maximum upwards recoil (rise)
	self.Primary.KickDown = 0.1 -- This is the maximum downwards recoil (skeet)
	self.Primary.KickHorizontal = 0 -- This is the maximum sideways recoil (no real term)
	self.Primary.StaticRecoilFactor = 0.5 --Amount of recoil to directly apply to EyeAngles.  Enter what fraction or percentage (in decimal form) you want.  This is also affected by a convar that defaults to 0.5.
end
...
--[[VIEWMODEL]]--
SWEP.ViewModel			= "models/weapons/tfa_cso2/c_elites.mdl" --Viewmodel path
...
--[[WORLDMODEL]]--
SWEP.WorldModel			= "models/weapons/tfa_cso2/w_elites_dropped.mdl" -- Weapon world model path
...
function SWEP:ChooseShootAnim( ... )
	if self:Clip1() == 2 then
		self:SendViewModelSeq( "Elite_shoot_leftlast" )
		return
	end
...
function SWEP:DrawWorldModel()
	...
	if not(  IsValid(ply) and ply:IsPlayer() ) then
		self:SetModel("models/weapons/tfa_cso2/w_elites_dropped.mdl")
		self:DrawModel()
		return
	end
	...
	if ( self.ShowWorldModel == nil or self.ShowWorldModel or not self:OwnerIsValid() ) then
		if game.SinglePlayer() or CLIENT then
			self:SetModel("models/weapons/tfa_cso2/w_elites_single.mdl")
