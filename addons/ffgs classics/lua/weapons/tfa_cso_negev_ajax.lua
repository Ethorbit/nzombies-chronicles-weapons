...
SWEP.Primary.DefaultClip			= 650				-- This is the number of bullets the gun gives you, counting a clip as defined directly above.
...
function SWEP:NZMaxAmmo()
	...
	if SERVER then
        ...
        self.Owner:SetAmmo( 15, self.Secondary.Ammo )
    end
end
...
SWEP.Secondary.Ammo	= "BULLET_AJAX"
game.AddAmmoType({
	name = "BULLET_AJAX",
	dmgtype = DMG_BLAST
})
...
function SWEP:PrimaryAttack(...)
	if (self:GetSilenced() and self.BlockOriginalFire and self:GetStatusEnd() <= CurTime()) then
		if (self:Ammo2() <= 0) then return end
		self:TakeSecondaryAmmo(1)

		local tr = util.TraceLine({
			start = self:GetOwner():EyePos(),
			endpos = self:GetOwner():GetAimVector() * 1000000,
			filter = function(ent) 
				return ent != self:GetOwner() and ent:GetClass() != "breakable_entry" and ent:GetClass() != "breakable_entry_plank" and ent:GetClass() != "invis_wall" and ent:GetClass() != "wall_block"
			end	
		})

		entsInRange = ents.FindInBox(tr.HitPos + Vector(95, 95, 95), tr.HitPos - Vector(95, 95, 95))
		local zombsKilled = 0
		for _,v in pairs(entsInRange) do
			if (IsValid(v) and v:IsValidZombie() and v:Health() > 0 and zombsKilled < 10) then
				local allowFX = false
				if (1600 >= v:Health()) then allowFX = true end
				...
				if (allowFX and zombsKilled < 5) then
					v:EmitSound("NPC_CombineBall.KillImpact")
				end

				zombsKilled = zombsKilled + 1
			end
		end
	elseif (self:CanPrimaryAttack()) then
		local chance = math.Round(util.SharedRandom("ElecZombie" .. tostring(CurTime()), 0, 100))
		if (chance <= 10) then
			local tr = util.TraceLine({
				start = self:GetOwner():EyePos(),
				endpos = self:GetOwner():GetAimVector() * 1000000,
			...
			if (IsValid(tr.Entity) and tr.Entity:IsValidZombie() and tr.Entity:Health() > 0) then
				if (1200 >= tr.Entity:Health()) then
					local effect = EffectData()
					effect:SetScale(1)
					effect:SetMagnitude(1)
					effect:SetScale(3)
					effect:SetRadius(1)
					effect:SetStart(tr.Entity:GetPos())
					effect:SetOrigin(tr.Entity:GetPos())
					effect:SetEntity(tr.Entity)
					effect:SetMagnitude(100)
					util.Effect("TeslaHitboxes", effect)
					tr.Entity:EmitSound("NPC_CombineBall.KillImpact")
				end
