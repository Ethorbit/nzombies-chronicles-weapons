...
if SERVER then
	...
	function ENT:PhysicsCollide(data, physobj)
		local plysToRevive = ents.FindInSphere(data.HitPos, 150)
		local revd = false
		for _,v in pairs(plysToRevive) do
			if (IsValid(self:GetOwner()) and IsValid(v) and v:IsPlayer() and !revd and !v:GetNotDowned() and v != self:GetOwner()) then
				revd = true
				v:RevivePlayer()
				hook.Call("PlayerRevived", nil, v, self:GetOwner()) 
			end
		end
