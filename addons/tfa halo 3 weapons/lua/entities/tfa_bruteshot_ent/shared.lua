	...
	local tr = util.TraceLine( trace )
		...
		if tr.Hit then
				local effectdata = EffectData()
					...
					util.Effect( "tfa_h3_bruteshot_explode", effectdata )
					util.BlastDamage(self.Entity, self:OwnerGet(), tr.HitPos, 300, 170)
					util.Decal("Scorch", tr.HitPos + tr.HitNormal, tr.HitPos - tr.HitNormal)
					self.Entity:SetVar("smoke", false)
					
			self:Explosion()
			self.Entity:EmitSound("bruteshot/elite_shot_explode_0"..math.random(1,6)..".wav", SNDLVL_110dB, 100, 0.8)
			self.Entity:Remove()
		end
...
if CLIENT then
 ...
 function ENT:Think()
	if (self.Entity:GetVar("smoke")) then
	pos = self:GetPos()
		for i=1, (1) do
			...
			if (particle) then
				...
				particle:SetDieTime( math.Rand( 1.1, 0.2 ) )
