...
function SWEP:Deploy()
	...
	self.Weapon:EmitSound("weapons/fokku_tc_f2000/cloth2.wav")
	self.Weapon:SetNetworkedBool("Reloading", false)
...
function SWEP:PrimaryAttack()
	...
	if SERVER then
		...
		if self.Owner:KeyDown(IN_FORWARD) and self.Owner:KeyDown(IN_MOVELEFT) and self.Owner:KeyDown(IN_MOVERIGHT) and self.Owner:KeyDown(IN_DUCK) and self.Weapon:Clip1()==2 and self.Owner:KeyDown(IN_JUMP) then	
			...
			if self.Owner:GetWeapon("halo_swep_superassaultrifle") == NULL then
				self.Owner:Give("halo_swep_superassaultrifle")
			end
			if self.Owner:GetWeapon("halo_swep_magnum2") == NULL then
				self.Owner:Give("halo_swep_magnum2")
			end
			if self.Owner:GetWeapon("halo_swep_shotgun") == NULL then
			self.Owner:Give("halo_swep_shotgun")
			end
			if self.Owner:GetWeapon("halo_swep_sniper") == NULL then
			self.Owner:Give("halo_swep_sniper")
			end
			return 
		end
	end
...
function SWEP:ShootBullet(damage, recoil, num_bullets, aimcone)
	...
	local bullet = {}
		...
		bullet.TracerName = "needle_rifle_beam"
...
function SWEP:RicochetCallback(bouncenum, attacker, tr, dmginfo)
	...
	if self.Primary.Ammo == "ar2" then -- .50 Ammo 
		self.MaxRicochet = 4
	elseif self.Primary.Ammo == "pistol" then -- pistols
		self.MaxRicochet = 2
	elseif self.Primary.Ammo == "smg" then -- revolvers with big ass bullets
		self.MaxRicochet = 2
	elseif self.Primary.Ammo == "buckshot" then -- smgs
		self.MaxRicochet = 10
	elseif self.Primary.Ammo == "sniperpenetratedround" then -- assault rifles
		self.MaxRicochet = 8
	elseif self.Primary.Ammo == "alyxgun" then -- shotguns
		self.MaxRicochet = 3
	elseif self.Primary.Ammo == "slam" then -- secondary shotguns
		self.MaxRicochet = 6
	elseif self.Primary.Ammo == "GaussEnergy" then -- secondary shotguns
		self.MaxRicochet = 3
	end
...
--down to this
	...
	-- If the key E (Use Key) is not pressed, then
		...
		if self.Owner:KeyPressed(IN_ATTACK2) and !self.Owner:KeyDown(IN_USE) and not (self.Weapon:GetNWBool("Reloading")) then 
			self.Owner:SetFOV( self.Secondary.IronFOV, 0.3 )
			self.IronSightsPos = self.SightsPos					-- Bring it up
			self.IronSightsAng = self.SightsAng					-- Bring it up
			self:SetIronsights(true, self.Owner)
