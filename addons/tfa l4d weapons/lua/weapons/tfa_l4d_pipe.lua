...
SWEP.Purpose				= ""
SWEP.Instructions				= ""
SWEP.PrintName				= "Pipe Bomb"		-- Weapon name (Shown on HUD)	
...
SWEP.ViewModel				= "models/wick/weapons/l4d1/c_pipebomb.mdl"	-- Weapon view model
SWEP.WorldModel				= "models/wick/weapons/l4d1/w_pipebomb.mdl"	-- Weapon world model
...
SWEP.Primary.Round 			= ("tfa_l4d1_pipe")	--NAME OF ENTITY GOES HERE
...
SWEP.EventTable = {
	[ACT_VM_DRAW] = {
		...
		{time = 0, type = "lua", value = function( wep, viewmodel ) ParticleEffectAttach("weapon_pipebomb_fuse_fp",PATTACH_POINT_FOLLOW,viewmodel,1) end, client = true, server = true},
	},
	[ACT_VM_HOLSTER] = {
		{time = 0, type = "sound", value = Sound("TFA_L4D1_UZI.DRAW")},
	}
...
SWEP.SprintAnimation = {
	["loop"] = {
		...
		["value"] = "pipe_run", --Number for act, String/Number for sequence
		["is_idle"] = true
	},	
}
...
SWEP.WalkAnimation = { 
	["loop"] = {
		["type"] = TFA.Enum.ANIMATION_SEQ, --Sequence or act
		["value"] = "pipe_walk", --Number for act, String/Number for sequence
		["is_idle"] = true
	},
}
...
function SWEP:DoAmmoCheck()
	if IsValid(self) and SERVER then
		...
		timer.Simple(delay, function()
			if IsValid(self) then
				self:Deploy()
				if IsValid(self) and self.Owner:IsValid() and self.Owner:GetViewModel():GetModel()==self.ViewModel then
				ParticleEffectAttach("weapon_pipebomb_fuse_fp",PATTACH_POINT_FOLLOW,self.Owner:GetViewModel(),1)
				end
			end
		end)
	end
