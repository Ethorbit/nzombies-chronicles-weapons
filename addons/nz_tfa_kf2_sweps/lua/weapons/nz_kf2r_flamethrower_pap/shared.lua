...
SWEP.PrintName						= "Firebug's Wraith"							-- Weapon name (Shown on HUD)
SWEP.NZPaPName						= "Firebug's Wraith"							-- Pack-a-Punch Name
...
--PaP Camo fixes (for scopes and things you dont want camos covering)
--SWEP.PaPMats = { 1 }
...
SWEP.Primary.RPM 					= 1040 											-- This is in Rounds Per Minute / RPM
...
SWEP.Primary.DefaultClip 			= 750 											-- This is the number of bullets the gun gives you, counting a clip as defined directly above.
SWEP.Primary.Ammo 					= "kf2r_flame" 									-- What kind of ammo.  Options, besides custom, include pistol, 357, smg1, ar2, buckshot, slam, SniperPenetratedRound, and AirboatGun.
SWEP.Primary.AmmoConsumption 		= 1 											-- Ammo consumed per shot
SWEP.Primary.MaxAmmo 				= 750
...
SWEP.VMPos 							= Vector(4.8, 5, -5)							-- The viewmodel positional offset, constantly.  Subtract this from any other modifications to viewmodel position.
...
if IsValid(b.Entity) and b.Entity.Ignite and not b.Entity:IsWorld() then
	b.Entity:Ignite( c:GetDamage(), 30 )
end
end
function SWEP:ShootBullet()
    if SERVER then
        local nearents = ents.FindInCone(self.Owner:EyePos(), self.Owner:GetAimVector() * 120, 200, 100)
        for _,ent in pairs(nearents) do
            if (nzConfig.ValidEnemies[ent:GetClass()] and ent:Alive()) then
                local tr = util.TraceLine({
                    start = ent:GetPos(),
                    endpos = self.Owner:EyePos(),
                    filter = function(ent2) 
                        return !ent2:IsWorld() and ent2 != ent and type(ent) != "nextbot" and ent2:GetClass() != "breakable_entry" and ent2:GetClass() != "invis_wall" and ent2:GetClass() != "invis_wall_zombie" and ent2:GetClass() != "wall_block" and ent2:GetClass() != "wall_block_zombie"
                    end,
                    collisiongroup = COLLISION_GROUP_PLAYER
                })
 
                if (tr.Entity != self.Owner) then -- Nothing's obstructing the zombie
                    ent:Ignite(10)
                    if (ent.m2burnmeagainin == nil or CurTime() > ent.m2burnmeagainin) then
                        initialdmg = DamageInfo()
                        initialdmg:SetDamageType(DMG_BURN)
 
                        if (self:HasNZModifier("pap")) then
                            initialdmg:SetDamage(45)
                        else
                            initialdmg:SetDamage(30)
                        end
            
                        initialdmg:SetAttacker(self.Owner)
                        ent:TakeDamageInfo(initialdmg)
                        ent.m2burnmeagainin = CurTime() + 0.5
                    end
 
                    if (timer.Exists("Burning" .. ent:EntIndex())) then return end -- They're already burning to death
                    timer.Create("Burning" .. ent:EntIndex(), 1, 10, function() -- Deal afterburn damage
                        if (!IsValid(ent) || !ent:Alive()) then
                            timer.Destroy("Burning" .. ent:EntIndex())
                        return end
 
                        local afterburn = DamageInfo()
                        afterburn:SetDamageType(DMG_BURN)
                        afterburn:SetDamage(150)
                        afterburn:SetAttacker(self.Owner)
 
                        if IsValid(ent) and ent:Alive() then
                            ent:TakeDamageInfo(afterburn)
                        end
                    end)
                end
            end
